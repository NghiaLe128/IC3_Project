...
const regexStart = /function startCatchPokemonSequence\(\) \{[\s\S]*?if \(!overlay\) return;/m;
let match = code.match(regexStart);
if (match) {
...
`;
    ...
    // Replace the rest of the function (since we redefined it, we have to slice it properly)
    code = code.replace(regexStart, match[0]); // actually, better to replace from img.src down to overlay.classList.remove("hidden");
}

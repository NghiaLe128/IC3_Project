...
// 1. Transform the form wrapper
code = code.replace(
  /<div class="glassmorphism p-8 rounded-3xl shadow-2xl relative">/,
  `<div class="glassmorphism rounded-3xl shadow-2xl relative overflow-hidden">
          <div id="loginSlider" class="flex w-[200%] transition-transform duration-500 ease-in-out" style="transform: translateX(0%);">
            <!-- View 1: Main Login -->
            <div class="w-1/2 flex-shrink-0 p-8 relative">`
);

// 2. Find the end of view 1 and insert view 2
code = code.replace(
  /            <\/div>\n          <\/div>\n\n        <\/div>\n      <\/div>\n    <\/main>/,
  `            </div>
          </div>
...
code = code.replace(/document\.getElementById\('googleSheetModal'\)\.classList\.remove\('hidden'\);/, "document.getElementById('loginSlider').style.transform = 'translateX(-50%)';");

code = code.replace(/document\.getElementById\('googleSheetModal'\)\.classList\.add\('hidden'\);/, "document.getElementById('loginSlider').style.transform = 'translateX(0%)';");

// 4. Remove the old modal
code = code.replace(/<!-- Google Sheet Student Login Modal -->[\s\S]*?<\/div>\n    <\/div>\n/g, "");
